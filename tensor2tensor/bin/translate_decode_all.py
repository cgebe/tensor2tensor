...
def main():
    for problem in TRANSLATE_PROBLEMS:
        for corpus in CORPORA:
            if os.system("python ./t2t-decoder --data_dir=$DATA_DIR/translate/"+problem+" --output_dir=$TRAIN_DIR/translate/"+problem+" --model=multi_model --hparams_set=multimodel_legal --problems="+problem+" --decode_hparams='batch_size=2,beam_size=4,alpha=0.6' --decode_from_file=$DECODE_DIR/"+getTestFile(corpus, problem)+" --decode_to_file=$DECODE_DIR/"+getDecodeFile(corpus, problem)) == 0:
                continue
            else:
                print "ERROR " + problem
                break
