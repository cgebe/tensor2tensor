...
def main():
    for problem in TRANSLATE_PROBLEMS:
        ...
        cmd = "python ./t2t-trainer --data_dir=$DATA_DIR --output_dir=$TRAIN_DIR/transformer/translate/"+problem+" --worker_gpu=8 --training_steps=250000 --model=transformer --hparams_set=transformer_base --save_checkpoints_secs=1200 --schedule=train --problems="+problem
        if os.system(cmd) == 0:
            continue
