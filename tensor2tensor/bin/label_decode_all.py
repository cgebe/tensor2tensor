...
def main():
    for problem in LABEL_PROBLEMS:
        ...
        cmd = "python ./t2t-decoder --data_dir=$DATA_DIR/multi_labeling --output_dir=$TRAIN_DIR/multi_labeling/"+problem+" --model=multi_model --hparams_set=multimodel_legal --problems="+problem+" --decode_hparams='batch_size=8,beam_size=4,alpha=0.6' --decode_from_file=$DECODE_DIR/multi_labeling/"+decode_file+" --decode_to_file=$DECODE_DIR/multi_labeling/single"
        os.system(cmd)
